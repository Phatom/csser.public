...
struct FileNames
{
	...
	{
		outl<<"------------------------------------------------------------\n";
		outl<<"Equilibration Session Files\n";
		outl<<"------------------------------------------------------------\n";
...
int dataRunner(int rank, string path, char* argv)
{
	...
	//4) Setup Hamiltonian
	bhamiltonian = new BoseHubbardClean(fnames.hamParamFile,stdo);
	...
	datarun->setOutputHeader("Bin\t\tStep\t\tOperator Count\t\tDensity\t\tEnergy\n");
	...
	if(dataparam.mode == SPDM || dataparam.mode == SPDMANDSFLUID)
	{
