...
long BasicSimEngine::loopUpdate(long long& loopLength)
{
	long numBounces=0;
	...
	if(stateVariable->nOp>0)
	{
		...
		for(long j=0;j<stateVariable->lpContrIter;j++)
		{
			...
			do{
				...
				vtxC = 4*p + exl;
				if(vtxC!=vtx0) //If it's reached then no vertex update of next vertex is required
				{
					vtxC = stateVariable->link[vtxC]; //Next vertex will be this
			...
			//Calculate average loop length. This will be needed during equilibration only.
